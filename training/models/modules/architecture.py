...
class RRDBNet(nn.Module):
    def __init__(self, in_nc, out_nc, nf, nb, gc=32, upscale=4,
        ...
        self.feature_extraction = nn.Sequential(
            B.conv_block(in_nc, nf, kernel_size=3,
                         norm_type=None, act_type=None)
        )

        # 2. Residual in Residual Dense Blocks (RRDB)
        self.residual_blocks = nn.ModuleList([
            B.RRDB(nf, kernel_size=3, gc=32, stride=1, bias=True,
        ...
        self.lr_conv = B.conv_block(nf, nf, kernel_size=3,
                                    norm_type=norm_type,
        ...
        self.hr_conv0 = B.conv_block(nf, nf, kernel_size=3,
                                     norm_type=None,
                                     act_type=act_type)

        self.hr_conv1 = B.conv_block(nf, out_nc, kernel_size=3,
                                     norm_type=None,
                                     act_type=None)

        self.hr_conv2 = B.conv_block(nf, nf, kernel_size=3,
                                     norm_type=None,
                                     act_type=act_type)

        self.hr_conv3 = B.conv_block(nf, out_nc, kernel_size=3,
                                     norm_type=None,
                                     act_type=None)


    def _create_upsampler(self, nf, upscale, upsample_mode, act_type):
        # Select upsampling block type
        if upsample_mode == 'upconv':
            upsample_block =  B.upconv_blcok
        elif upsample_mode == 'pixelshuffle':
            upsample_block = B.pixelshuffle_block
